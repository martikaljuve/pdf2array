...
function _applyVerticalSliceToPage(rows: Row[], options?: ApplySliceOptions) {
	...
	const colXs = localMinima
		...
		.concat([
			[((localMinima[localMinima.length - 1] ?? 0) * width) / verticalSlices, Infinity],
		]);
		...
		for (let [colStart, colEnd] of colXs) {
			...
			while (!!row.items[itemIdx] && row.items[itemIdx].x < colEnd) {
				...
				newItem = {
					...
					str:
						newItem.str.length && item.str.length
							? `${newItem.str} ${item.str}`
							: item.str,
				};
