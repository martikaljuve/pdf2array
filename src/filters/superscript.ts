...
export function stripSuperscripts(rows: Row[], options?: StripSuperscriptOptions) {
	...
	for (let page of pages) {
		...
		const topLeftTree = (() => {
			if (stripLeft) {
				const topLeftPoints = items.map((item) => [item.x, item.y]);
		...
		const topRightTree = (() => {
			if (stripRight) {
				const topRightPoints = items.map((item) => [item.x + item.width, item.y]);
		...
		items.forEach((item, i) => {
			...
			const topLeftMatches =
				topLeftTree
					?.radiusSearch(rightPos, item.height * radiusScale)
					?.filter((idx) => idx !== i)
					?.filter((idx) => items[idx].x > item.x)
					?.filter((idx) => items[idx].height * heightScale > item.height) ?? [];

			const topRightMatches =
				topRightTree
					?.radiusSearch(leftPos, item.height * radiusScale)
					?.filter((idx) => idx !== i)
					?.filter((idx) => items[idx].x < item.x)
					?.filter((idx) => items[idx].height * heightScale > item.height) ?? [];
