...
function _cmpFooters(a: Row, b: Row, options?: StripFootersOptions) {
	...
	for (let i = 0; i < a.items.length; ++i) {
		...
		if (
			Math.abs(aItem.x - bItem.x) > xTolerance &&
		...
		if (
			aItem.str.replaceAll(/\d+/g, '').trim().toLowerCase() !==
...
export function stripFooters(rows: Row[], options?: StripFootersOptions) {
	...
	const m = footers.map((ri, i) =>
		footers.map((rj, j) => (j < i ? _cmpFooters(ri, rj) : undefined)),
	);
	...
	if (largestCluster !== undefined) {
		if (
			largestCluster[1] / footers.length >
			(options?.confidence ?? DefaultFooterOptions.confidence)
		) {
